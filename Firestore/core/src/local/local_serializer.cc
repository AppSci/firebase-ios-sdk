...
MutationBatch LocalSerializer::DecodeMutationBatch(
  ...
  for (size_t i = proto.writes_count - 1; i >= 0; --i) {
    ...
    if (mutation.transform.field_transforms_count > 0) {
      printf("assert:, %d, %d, %d", i >= 1, proto.writes[i - 1].transform.field_transforms_count == 0, proto.writes[i - 1].update.fields_count > 0);
      HARD_ASSERT(
          i >= 1 && proto.writes[i - 1].transform.field_transforms_count == 0 &&
              proto.writes[i - 1].update.fields_count > 0,
